...
namespace BITSharp
{
    public partial class BitClient
    {
        ...
        /// <summary>
        /// On Sale Now Request - Returns events going on sale in the next week (including today). Supports location filtering.
        /// </summary>
        /// <param name="location">A location option for searching {city,state (US or CA)} {city,country} {lat,lon} {ip address} {"use_geoip"}</param>
        /// <param name="radius">Radius around the location to search (Miles, Max 150) [Default: 25]</param>
        /// <returns></returns>
        public List<Models.Event> Events_OnSaleSoon(string location, int? radius)
        {
            var request = new RestRequest();
            request.Resource = "events/on_sale_soon.xml";
